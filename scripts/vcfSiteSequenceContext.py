...
class AdjacentSubMergeReader(vcf.Reader):
    ...
    def _doMerge(self, this, that):
        ...
        - `that`:
        """
        assert len(this.alleles) == 2, this.alleles
        assert len(that.alleles) == 2, that.alleles
...
class SiteSequenceContext(object):
    ...
    def iterContextMutations(self, context=1, contig=None):
        ...
        for site in self.vcf:
            ...
            chrom, pos, refA = site.CHROM, site.POS - 1, str(site.REF)
            try:
                ...
                continue
            refCtx = refSeq[
                int(pos - context) : int(pos + len(refA) + context)
            ].decode()
            refCtx = refCtx.upper()
    ...
    def write(self, outStrm, renaming={}):
        """
        ...
        outStrm.write(
            "#mutType\t" + "\t".join(renaming.get(x, x) for x in samples) + "\n"
        )
        for mType in mTypes:
            outStrm.write(
                "%s>%s\t" % (mType)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rename_sample",
        help="Rename sample OLD:NEW.  Old name for unnamed sample is REF. Can be given multiple times [default:%(default)s]",
        action="append",
        default=[],
    )
    ...
    if args.vcf in ("stdin", "/dev/stdin", "-"):
        args.vcf = sys.stdin
    ...
    if args.out in ("-", "stdout"):
        logging.info("Outputting to standard output")
        outStrm = sys.stdout
    else:
        logging.info("Outputting to %s" % (args.out))
    ...
    if args.bedout:
        ...
        renamings = {
            OLD: NEW for OLD, NEW in (x.split(":") for x in args.rename_sample)
        }
        for OLD, NEW in renamings.items():
            logging.info("Renaming %s to %s" % (OLD, NEW))
        seqContext.countSpectrum(args.context, args.contig)
        seqContext.write(outStrm, renamings)
